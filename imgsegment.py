...
COLORS = np.array([
    ...
    (0, 0, 255),    # Màu xanh lam (rất lạnh)
    (0, 128, 255),  # Màu xanh da trời nhạt (lạnh)
    (0, 255, 255),  # Màu xanh lục nhạt (mát)
    (0, 255, 128),  # Màu xanh lá cây nhạt (mát mẻ)
    (0, 255, 0),    # Màu xanh lá cây (ấm áp)
    (255, 255, 0),  # Màu vàng (ấm)
    (255, 128, 0),  # Màu cam (ấm nóng)
    (255, 69, 0),   # Màu cam đậm (nóng)
    (255, 0, 0),    # Màu đỏ (rất nóng)
    (128, 0, 0)     # Màu đỏ sẫm (cực kỳ nóng
], dtype=np.uint8)
...
class DimgSegment:
    ...
    def draw_gray_from_data(self, data: np.ndarray, height: int, width: int, fileio: str, format: str = 'JPEG'):
        out_shape = (height, width)
        segmented_image = data.reshape(out_shape)
        return self.draw_image_from_data(segmented_image, fileio=fileio, format=format)

    def save_label2image(self, labels: np.ndarray, height: int, width: int, fileio: str, format: str = 'JPEG'):  # TIFF
        ...
        return self.draw_image_from_data(data=colored_segmented_image, fileio=fileio, format=format)
